...
public class Mic1Controller
{
    ...
    private bool _enablePerformanceCount;
    ...
    private void HelpCommand(string[] args)
    {
        ...
        WriteLabelValue("perf         n                           - ",
            "Sets the interval for collecting performance stats.");
        WriteLabelValue("stop                                     - ",
            "Halt any code currently executing on the MIC-1 and return to idle.");
        WriteLabelValue("start                                    - ",
            "Move out of idle mode and start executing code at the current PC (Program Counter).");
        WriteLabelValue("time                                     - ", "Show the current date/time.");
        WriteLabelValue("set          <flag> on/off               - ",
            "Toggles the specific flag that effects behaviour of the simulator. Use help set for more information.");
        WriteLabelValue("dumpmem      [f] [<start>] [<length>]    - ",
            "Dumps memory in the specified format starting at <start>, showing <length> bytes.");
        WriteLabelValue("info         [f]                         - ", "Show information about the MIC-1 Simulator.");
        WriteLabelValue("dumpstack    [f] [sw]                    - ",
            "Dump the stack in specified format: decimal, hex, binary, or octal. Use sw to show as words.");
        WriteLabelValue("stats        [f]                         - ",
            "Show Detailed Processor Stats in specified format.");
        WriteLabelValue("perf         [f]                         - ",
            "Trace Processor performance in specified format.");
        WriteLabelValue("showerrors   [n]                         - ",
            "Show error the last n errors generated by the simulator. Omit n for all errors.");
        WriteLabelValue("clearerrors                              - ", "Clears the processor error collection.");
        WriteLabelValue("(h)elp       [command]                   - ",
            "Show help, optionally for a specific [command].");
        WriteLabelValue("stats        [d]                         - ",
            "Display information about the simulator. [d] for more detailed stats.");
        WriteLabelValue("reset                                    - ", "Reset the simulator to its initial state.");
        WriteLabelValue("perfmon                                  - ",
            "Enable the performance monitor. When next in run mode data will stream to the console.");
    ...
    private void InitializeCommands()
    {
        _helpFiles = new Dictionary<string, Action>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            [CommandType.RunPerf.GetDescription()] = DisplayPerfMonText,
        ...
        _commands = new Dictionary<string, Action<string[]>>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            [CommandType.PerfInterval.GetDescription()] = SetPerfIntervalCommand,
    ...
    private void InitialiseEvents()
    {
        ...
        MIC1.TrapEvent += (_, args) => { Errors.Add(args); };
    ...
    private void DisplayMemDumpText()
    {
        ...
        Console.WriteLine(
            "Dumps memory in the specified format starting at <start>, showing <length> bytes. Format is optional but will display all values as specified.");
    ...
    private void DisplayStatsText()
    {
        ...
        Console.WriteLine(
            "Display statistics about the simulator. The [d] switch can be used to display detailed instructions about IJVM execution.");
    ...
    private void DisplayPerfMonText()
    {
        DisplayCommandText("perfmon.");
        Console.WriteLine("Starts the performance monitor.");
        Console.WriteLine(
            "This automatically enables processor event generation. If the processor is currently in idle mode then no stats will appear until it is started (see help start).");
        Console.WriteLine(
            "To stop the monitor hold down the ESC key. NOTE this leaves the simulator in Event mode if it was not already.");
        Console.WriteLine("Once disabled in this manner issuing the perfmon command again will reenable it.");
        Console.WriteLine();
        Console.WriteLine("EXAMPLES: perfmon");
    }
    ...
    private void DisplaySetText()
    {
        ...
        Console.WriteLine(
            "Performance interval is the gap to report processor stats. Default is 2 seconds and Events must be on for this to work.");
        Console.WriteLine();
    ...
    private void DisplayStopText()
    {
        ...
        Console.WriteLine(
            "Halt any code currently executing on the MIC-1 and return to idle. This actually stops the acculation of cycles and processor time.");
        Console.WriteLine();
    ...
    {
        DisplayCommandText("dumpstack [f] [sw]");
        Console.WriteLine("Displays a listing of the stack, from the top of the stack to the current stack pointer.");
        Console.WriteLine(FormatParameterHelpText);
        Console.WriteLine(
            "Optional parameter sw will show the stack as 2 16 bit words to align with its logical structure.");
        Console.WriteLine();
    ...
    {
        Console.WriteLine("Performance monitor is enable. When processor is not idle, data will stream to console. Hold ESC to stop.");
        MIC1.EnableExecutionEvents = true;
        _enablePerformanceCount = true;
    }

    private void SetPerfIntervalCommand(string[] args)
    {
        ...
        {
            Console.WriteLine("Valid values are 1-10 (time is in seconds).");
            return;
        }
    ...
    private void ShowErrorsCommand(string[] args)
    {
        ...
        {
            Console.WriteLine("{0,-20:yyyy-MM-dd HH:mm:ss} | {1,-25} | {2}", error.DateTime, error.TrapCode,
                error.Message);
    ...
    private void InfoCommand(string[] args)
    {
        ...
        WriteLabelValue("Processor Speed (MHz):\t\t",
            MIC1.UnThrottled
                ? "Max Possible"
                : (MIC1.TargetProcessorSpeed / 1e6).ToString(CultureInfo.InvariantCulture));
    ...
    private void DumpMemoryCommand(string[] args)
    {
        ...
        var intArgsStart = args.Length > 0 && args[0].IsInteger() ? 0 : 1;
    ...
    private void DumpStackCommand(string[] args)
    {
        ...
        if (showWords)
        {
            var rows = new List<(string Address, string HighWord, string LowWord, string Value)>();
            
            BuildStackAsWords(rows, format);

            var padAddressWidth = ComputeWidth(rows.Select(r => r.Address));
            var padHiWidth = ComputeWidth(rows.Select(r => r.HighWord));
            var padLoWidth = ComputeWidth(rows.Select(r => r.LowWord));
            
            WriteLabel("Address".PadRight(padAddressWidth));
            WriteLabel("High Word".PadRight(padHiWidth));
            WriteLabel("Low Word".PadRight(padLoWidth));
            WriteLabel("Value");
            Console.WriteLine();

            rows.ForEach(d =>
            {
                WriteValue(d.Address.PadRight(padAddressWidth));
                WriteValue(d.HighWord.PadRight(padHiWidth));
                WriteValue(d.LowWord.PadRight(padLoWidth));
                WriteValue(d.Value);
                Console.WriteLine();
            });
        }
    ...
    private static void BuildStackAsWords(List<(string Address, string HighWord, string LowWord, string Value)> rows,
        NumberFormat format)
    {
        for (var address = Registers.StackStart - 1; address >= Registers.SP; address -= 4)
        {
            // Big-endian read: MSB first
            var lsb2 = Memory.ReadByte(address);     // byte 0 (MSB)
            var lsb1 = Memory.ReadByte(address - 1); // byte 1
            var msb2 = Memory.ReadByte(address - 2); // byte 2
            var msb1 = Memory.ReadByte(address - 3); // byte 3 (LSB)

            // Compose values
            var highWord = (ushort)((msb1 << 8) | msb2);
            var lowWord = (ushort)((lsb1 << 8) | lsb2);
            var fullValue = (msb1 << 24) | (msb2 << 16) | (lsb1 << 8) | lsb2;

            // Format values
            var addressString = (address - 3).FormatValue(format);
            var highStr = highWord.FormatValue(format);
            var lowStr = lowWord.FormatValue(format);
            var valueStr = fullValue.FormatValue(format);

            // Annotate value if this is the stack pointer
            if ((address - 3) == Registers.SP)
            {
                valueStr += " <-- SP";
            }

            // Add a row with all components
            rows.Add((addressString, highStr, lowStr, valueStr));
        }
    }
    private static void BuildStackAsBytes(List<(int Address, string Label, string Value)> rows, NumberFormat format)
    {
        for (var address = Registers.StackStart - 1; address >= Registers.SP; address--)
        {
            var value = Memory.ReadByte(address);
